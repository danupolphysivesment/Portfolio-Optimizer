...
def sidebar_settings() -> dict:
    ...
    section = st.sidebar.radio(
        "Workbench",
        ["🧮 Optimizer", "🧪 Backtester", "🎲 Monte Carlo", "📚 Asset Universe", "📖 Methods"],
...
def section_montecarlo(cfg: dict):
    ...
    m[1].metric("90% range", f"${stats['p5 terminal']:,.0f} – ${stats['p95 terminal']:,.0f}")
    ...
    # ---- Cholesky / correlation validation -------------------------------
    with st.expander("🔗 Correlation preservation (Cholesky check)", expanded=False):
        st.markdown(
            "Shocks are generated as **L · z**, where **L** is the Cholesky factor of "
            "the covariance (Σ = L · Lᵀ) and z are independent standard normals. This "
            "rotates uncorrelated noise into the historical correlation structure."
        )
        st.caption(
    ...
    st.caption(
        "⚠️ Projections assume returns and correlations stay like the chosen history. "
        "Expected returns are especially uncertain — use the drift haircut to stay "
        "conservative. Educational, not investment advice."
    )
