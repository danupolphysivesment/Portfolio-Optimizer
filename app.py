...
def load_prices_cached(tickers: tuple, start: str, end: str, source: str):
    return dataio.load_prices(list(tickers), start, end, source=source)
...
def asset_picker(key: str, default=None) -> list:
    """Three grouped multiselects (one per asset class)."""
    default = default or uni.default_selection()
    chosen = []
    cols = st.columns(3)
    for col, ac in zip(cols, uni.ASSET_CLASSES):
        with col:
            tickers = [a.ticker for a in uni.assets_in_class(ac)]
            picked = st.multiselect(
                ac,
                tickers,
                default=[t for t in tickers if t in default],
                help="; ".join(uni.label(t) for t in tickers),
                key=f"{key}_{ac}",
            )
...
def weight_area_fig(weight_df: pd.DataFrame) -> go.Figure:
    wd = weight_df.dropna(how="all").resample("M").last().dropna(how="all")
    fig = go.Figure()
    for t in wd.columns:
        fig.add_trace(go.Scatter(
...
def section_optimizer(cfg: dict):
    ...
    prices, warns = load_prices_cached(tuple(tickers), cfg["start"], cfg["end"], cfg["source"])
    for wmsg in warns:
        st.warning(wmsg)
...
def section_universe(cfg: dict):
    ...
    st.markdown(
        '<span class="pl-sub">Fifteen liquid ETFs across three asset classes. '
        "Explore historical behavior and co-movement.</span>",
        unsafe_allow_html=True,
    )
    for ac in uni.ASSET_CLASSES:
        st.markdown(
            f'<span class="pl-tag" style="background:{CLASS_COLORS[ac]}">{ac}</span>',
            unsafe_allow_html=True,
        )
        rows = [{"Ticker": a.ticker, "Description": a.name}
                for a in uni.assets_in_class(ac)]
