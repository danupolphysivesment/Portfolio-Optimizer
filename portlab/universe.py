...
def default_selection() -> List[str]:
    """Global country-index starter basket.

    Equity: ACWI (world) + one equity index per country/region.
    Fixed Income: one ETF per bond type + the US aggregate.
    Alternatives: gold, oil, and REITs.
    """
    return [
        # Equity — ACWI world + country/regional indices
        "ACWI", "SPY", "EXW1.DE", "EWJ", "EWY", "EWT", "INDA", "VNM", "THD",
        # Fixed Income — each bond type + US aggregate
        "AGG", "TLT", "LQDA.L", "HYG", "TIP", "FLOA.L", "IMBA.L",
        # Alternatives — gold, oil, REITs
        "GLD", "USL", "REET",
    ]
