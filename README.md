...
### 🎲 Monte Carlo
Project any portfolio forward with thousands of random scenarios. Correlated
shocks are generated via **Cholesky decomposition** of the historical covariance
(Σ = L·Lᵀ, so the simulated cross-asset correlations match the real ones), with
geometric Brownian motion per asset and an optional **Student-t** shock for fat
tails. A drift-haircut control shrinks noisy expected returns toward the
risk-free rate.

Organized into three tabs:
- **📈 Projection** — a **fan chart** (5–95% and 25–75% bands around the median
  path), terminal-value distribution, probability of loss, probability of hitting
  a return goal, terminal VaR/CVaR, simulated max-drawdown distribution, and a
  built-in check that the Cholesky coupling reproduced the input correlation.
- **📉 VaR · CVaR · EVT** — value-at-risk and expected shortfall estimated four
  ways (**Historical, Gaussian, Cornish-Fisher, and Extreme Value Theory**). EVT
  fits a Generalized Pareto Distribution to the loss tail (peaks-over-threshold,
  McNeil–Frey), reports the tail index ξ, and plots the fitted vs empirical tail —
  showing how the normal model understates deep losses.
- **🔥 Stress tests** — **historical crisis replay** (the allocation's actual
  return through the GFC, COVID crash, 2022 rate shock, etc., when inside the data
  window) plus **hypothetical factor shocks** by asset class (preset and custom).

### 🌐 Macro Impulse Response
Model the portfolio jointly with six macro-factor proxies (S&P 500, Δ10-year
yield, US-dollar index, WTI crude, ΔVIX, gold) in a **vector autoregression
(VAR)**, then trace **orthogonalized impulse responses** — how the portfolio
reacts, period by period, to a 1σ shock in each factor. Shocks are identified by
a Cholesky ordering of the residual covariance (factors before the portfolio).
Outputs: cumulative impulse-response curves, the cumulative impact and **forecast
error variance decomposition (FEVD)** at the horizon, per-factor responses with
95% bands, and a static 1σ-sensitivity table.
