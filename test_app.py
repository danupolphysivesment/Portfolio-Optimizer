...
def main():
    ...
    for sec in ["🛰️ Exposure Monitor", "🧪 Backtester", "🎲 Monte Carlo",
                "🌐 Macro Impulse", "📚 Asset Universe", "📖 Methods"]:
        at = base_app()
        at.radio(key="cfg_section").set_value(sec).run()
        ok &= check(at, sec)

    # --- Macro: actually run the VAR / impulse-response analysis -------
    print("Macro — run impulse response:")
    at = base_app()
    at.radio(key="cfg_section").set_value("🌐 Macro Impulse").run()
    runbtn = [b for b in at.button if b.key == "macro_run"]
    if runbtn:
        runbtn[0].click().run()
        ok &= check(at, "impulse-response run")
    else:
        print("  ! macro run button not found")

    # --- Monte Carlo: actually run a simulation ------------------------
    print("Monte Carlo — run simulation:")
    at = base_app()
    at.radio(key="cfg_section").set_value("🎲 Monte Carlo").run()
    runbtn = [b for b in at.button if b.key == "mc_run"]
    if runbtn:
        runbtn[0].click().run()
        ok &= check(at, "simulation run")
    else:
